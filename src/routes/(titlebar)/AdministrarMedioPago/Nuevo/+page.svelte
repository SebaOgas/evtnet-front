<script lang="ts">
	...
	async function guardar() {
		...
		try {
			await MedioPagoService.altaMedioPago(data);
            exitoVisible = true;
		} catch (e) {
...
<div id="content">
	<div class="p-2 text-xs flex flex-col gap-2 overflow-y-auto grow md:grow-0">
		<h1 class="text-m text-center md:text-start">Alta de Medio de Pago</h1>
...
<PopupError bind:visible={errorPermiso}>
	No tiene permiso para dar de alta medios de pago.
</PopupError>

<Popup bind:visible={exitoVisible} fitH fitW>
	<span>
        Medio de pago añadido exitosamente. 
    </span>
    <div class="flex w-full justify-center">
        <Button action={() => goto("/AdministrarMedioPago")}>Aceptar</Button>
    </div>
</Popup>