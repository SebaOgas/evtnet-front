<script lang="ts">
	...
	$: data = {
		...
		nombre: "",
		descripcion: "",
	} as DTOModificarDisciplina;

	let original : DTODisciplina = {
		id: id,
		nombre: "",
		descripcion: "",
		fechaAlta: new Date(),
	...
	onMount(async () => {
		...
		try{
            original = await DisciplinasService.obtenerDisciplinaCompleta(id);
	...
	async function guardar() {
		...
		try {
			await DisciplinasService.modificarDisciplina(data);
            exitoVisible = true;
		} catch (e) {
			if (e instanceof HttpError) {
				error = e.message;
...
<PopupError bind:visible={errorPermiso}>
	No tiene permiso para modificar disciplinas.
</PopupError>

<Popup bind:visible={exitoVisible} fitH fitW>
	<span>
        Disciplina modificada exitosamente. 
    </span>
    <div class="flex w-full justify-center">
        <Button action={() => goto("/AdministrarDisciplinas")}>Aceptar</Button>
    </div>
</Popup>