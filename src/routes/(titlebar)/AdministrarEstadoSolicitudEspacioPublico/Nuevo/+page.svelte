<script lang="ts">
	...
	async function guardar() {
		...
		try {
			await EstadoSolicitudEspacioPublicoService.altaEstadoSolicitud(data);
            exitoVisible = true;
		} catch (e) {
...
<div id="content">
	<div class="p-2 text-xs flex flex-col gap-2 overflow-y-auto grow md:grow-0">
		<h1 class="text-m text-center md:text-start">Alta de EstadoSolicitud</h1>
...
<PopupError bind:visible={errorPermiso}>
	No tiene permiso para dar de alta estados de solicitud de espacio público.
</PopupError>

<Popup bind:visible={exitoVisible} fitH fitW>
	<span>
        EstadoSolicitud añadida exitosamente. 
    </span>
