<script lang="ts">
	...
	$: data = {
		...
		fechaDesde: null,
		fechaHasta: null,
	} as DTOModificarComision;

	let original : DTOComision = {
		id: id,
		montoLimite: 0,
		porcentaje: 0,
		fechaDesde: null,
	...
	onMount(async () => {
		...
		try{
            original = await ComisionService.obtenerComisionInscripcionCompleta(id);
		
            data.id = original.id;
            data.montoLimite = original.montoLimite;
            data.porcentaje = original.porcentaje;
            data.fechaDesde = original.fechaDesde;
            data.fechaHasta = original.fechaHasta;
			fechaDesde = original.fechaDesde ? new Date(original.fechaDesde) : null;
			fechaHasta = original.fechaHasta ? new Date(original.fechaHasta) : null;
        }catch(e){
	...
	async function guardar() {
		...
		try {
			await ComisionService.modificarComisionInscripcion(data);
            exitoVisible = true;
		} catch (e) {
			if (e instanceof HttpError) {
				error = e.message;
...
<div id="content">
	<div class="p-2 text-xs flex flex-col gap-2  grow md:grow-0">
		<h1 class="text-m text-center md:text-start">Modificar Comisión por Inscripción</h1>
...
<PopupError bind:visible={errorPermiso}>
	No tiene permiso para modificar Comisiones por Inscripción.
</PopupError>

<Popup bind:visible={exitoVisible} fitH fitW>
	<span>
        Comisión por Inscripción modificada exitosamente. 
    </span>
