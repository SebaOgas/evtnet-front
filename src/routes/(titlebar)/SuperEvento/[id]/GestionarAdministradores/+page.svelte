<script lang="ts">
	...
	$: data = null as DTOAdministradoresSuperevento | null;
    ...
    async function cargarDatos() {
        ...
        try {
			data = await SupereventosService.obtenerAdministradores(id);
	...
	async function confirmarAgregarAdmin() {
		...
		try {
			await SupereventosService.agregarAdministrador(id, adminSelec.username);
            adminSelec = null;
	...
	async function confirmarQuitarAdmin() {
        if (adminSelec === null) return;
		try {
			await SupereventosService.quitarAdministrador(id, adminSelec.username);
            adminSelec = null;
	...
	async function confirmarEntregarOrganizadorAdmin() {
        if (adminSelec === null) return;
		try {
			await SupereventosService.entregarOrganizador(id, adminSelec.username);
            adminSelec = null;
	...
	async function buscarUsuarios(valor: string) {
		return await SupereventosService.buscarUsuariosNoAdministradores(id, valor);
	}
...
<div id="content">
	...
	<div class="flex flex-wrap gap-2 h-fit p-2 justify-center items-center">
		<Button action={() => goto(`/SuperEvento/${id}/Administrar`)}>Atrás</Button>
	</div>
</div>
{/if}
...
<Popup title="Confirmar" bind:visible={popupOrganizadorConfirmacionVisible} fitW fitH>
	<div class="grow overflow-y-auto">
        ...
        <p>{adminSelec?.nombre}, {adminSelec?.apellido} ({adminSelec?.username})</p>
        <p>Usted ya no será organizador, sino que pasará a ser solo administrador del superevento.</p>
	</div>
	<div class="flex gap-2 h-fit p-2 justify-center items-center">
...
<PopupError bind:visible={errorPermiso} redir={previousPage}>
	No tiene permiso para administrar este superevento.
</PopupError>
