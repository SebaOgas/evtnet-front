<script lang="ts">
	...
	$: data = {
		...
		nombre: "",
		descripcion: "",
	} as DTOModificarModoEvento;

	let original : DTOModoEvento = {
		id: id,
		nombre: "",
		descripcion: "",
		fechaAlta: new Date(),
	...
	onMount(async () => {
		...
		try{
            original = await ModosDeEventoService.obtenerModoEventoCompleta(id);
	...
	async function guardar() {
		...
		try {
			await ModosDeEventoService.modificarModoEvento(data);
            exitoVisible = true;
		} catch (e) {
			if (e instanceof HttpError) {
				error = e.message;
...
<div id="content">
	<div class="p-2 text-xs flex flex-col gap-2 overflow-y-auto grow md:grow-0">
		<h1 class="text-m text-center md:text-start">Modificar Modo Evento</h1>
...
<PopupError bind:visible={errorPermiso}>
	No tiene permiso para modificar modos de evento.
</PopupError>
