<script lang="ts">
	...
	$: data = {
		...
		nombre: "",
		descripcion: "",
	} as DTOModificarEstadoDenuncia;

	let original : DTOEstadoDenuncia = {
		id: id,
		nombre: "",
		descripcion: "",
		fechaAlta: new Date(),
	...
	onMount(async () => {
		...
		try{
            original = await EstadoDenunciaEventoService.obtenerEstadoDenunciaCompleta(id);
	...
	async function guardar() {
		...
		try {
			await EstadoDenunciaEventoService.modificarEstadoDenuncia(data);
            exitoVisible = true;
		} catch (e) {
			if (e instanceof HttpError) {
				error = e.message;
	...
	function cancelar() {
		goto("/AdministrarEstadoDenunciaEvento");
	}
...
<div id="content">
	<div class="p-2 text-xs flex flex-col gap-2 overflow-y-auto grow md:grow-0">
		<h1 class="text-m text-center md:text-start">Modificar Estado Denuncia</h1>
...
<PopupError bind:visible={errorPermiso}>
	No tiene permiso para modificar Estados Denuncia.
</PopupError>
