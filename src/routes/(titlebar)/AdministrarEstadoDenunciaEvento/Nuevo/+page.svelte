<script lang="ts">
	...
	async function guardar() {
		...
		try {
			await EstadoDenunciaEventoService.altaEstadoDenuncia(data);
            exitoVisible = true;
		} catch (e) {
...
<div id="content">
	<div class="p-2 text-xs flex flex-col gap-2 overflow-y-auto grow md:grow-0">
		<h1 class="text-m text-center md:text-start">Alta de Estado Denuncia</h1>
...
<PopupError bind:visible={errorPermiso}>
	No tiene permiso para dar de alta estados de denuncia.
</PopupError>

<Popup bind:visible={exitoVisible} fitH fitW>
	<span>
        Estado Denuncia añadido exitosamente. 
    </span>
